...
- **Messaging**: text, photos, videos, voice messages (recorded on-device, encoded to Ogg/Opus so they render as real inline voice bubbles elsewhere, not generic file attachments), files, replies, emoji reactions, message editing, message deletion, typing indicators, read receipts.
...
- **Persistent login**: log in once; relaunching the app resumes the same session automatically (an expired/revoked token falls back to the login form on its own). Logging out clears it.
...
- **Notifications, including with the app fully closed**: BlackBerry Hub integration, including an explicit opt-in into Instant Preview (the pop-up banner style other Hub-integrated apps use), and disabled entirely during the very first sync so logging in doesn't produce a notification for every message in your history. A background service wakes on its own roughly every 6 minutes to check for new messages and post any that arrived — see [Architecture](#architecture) for how, and why it's a periodic wake rather than instant push.
...
- **Background notifications are a periodic wake, not instant push.** The headless service (see [Architecture](#architecture)) checks for new messages roughly every 6 minutes while the app is closed, not the moment a message arrives — open the app for immediate delivery.
...
- **Persistent login and background sync run as a second, headless binary, not a resident service.** The access token, device ID, and homeserver are cached to disk on login and reused via the same token-login path already used for "existing access token" logins, so a relaunch resumes the same Olm identity and E2EE session instead of starting fresh. Background delivery uses `BBportHeadless`, a separate ELF binary packaged into the same `.bar` (its own `entryPoint`/`invoke-target`, `bb::Application` rather than `bb::cascades::Application`, no QML), woken every 6 minutes — `InvokeRecurrenceRule`'s documented floor, the densest schedule the API allows — by `bb::system::InvokeManager::registerTimer()`. An always-resident headless process (`_sys_headless_nostop`) was tried first — it matches BlackBerry's own official sample structure and packages/installs fine, but never actually launches on a real device, because that permission historically required a per-app manual approval from BlackBerry that no longer exists now that BB10 is EOL. `_sys_run_headless` alone, for a short-running trigger-based invocation, doesn't need that approval, hence the timer instead of a resident daemon. A PID file (`~/foreground.pid`, checked with a `kill(pid, 0)` liveness probe) lets the headless process detect the UI app is already running and exit immediately, so the two never both hold a `/sync` connection at once.
