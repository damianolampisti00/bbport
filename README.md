...
- **Notifications, including with the app fully closed**: BlackBerry Hub integration, including an explicit opt-in into Instant Preview (the pop-up banner style other Hub-integrated apps use), and disabled entirely during the very first sync so logging in doesn't produce a notification for every message in your history. A background service wakes on its own roughly every 15 minutes to check for new messages and post any that arrived — see [Architecture](#architecture) for how, and why it's a periodic wake rather than instant push.
...
- **Background notifications are a periodic wake, not instant push.** The headless service (see [Architecture](#architecture)) checks for new messages roughly every 15 minutes while the app is closed, not the moment a message arrives — open the app for immediate delivery.
